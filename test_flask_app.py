...
class TestRunningTrainingPlanning(unittest.TestCase):
    ...
    def test_option_1_post(self):
        # Test POST for Option 1 (7.2 km with drills and sprints)
        ...
        self.assertIn(b'7.2 km (SCTC -> Sha Tin Sports Ground -> Lek Yuen Bridge -> SCTC)', response.data)
        self.assertIn(b'43 min', response.data)
        self.assertIn(b'12 reps (~6 min)', response.data)
        self.assertIn(b'4 x 100m (~7 min, includes rest)', response.data)
        self.assertIn(b'0 hr 56 min', response.data)
    ...
    def test_option_3_post(self):
        # Test POST for Option 3 (10.9 km, run only)
        response = self.app.post('/', data={
            'route': '3',
            'pace_minutes': '6',
            'pace_seconds': '0'
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Workout Summary', response.data)
        self.assertIn(b'10.9 km (SCTC -> Ma Bridge -> HK Science Park -> SCTC)', response.data)
        self.assertIn(b'65 min', response.data)
        self.assertIn(b'1 hr 5 min', response.data)
        self.assertNotIn(b'Drills:', response.data)
        self.assertNotIn(b'Sprints:', response.data)
